...
<body>
    <!--标题部分-->
    <div id="divTitle">
        <span class="title"><a href='<%=Url.Action("Index") %>'>销售机会管理</a>&nbsp;>>&nbsp;修改销售机会</span><br />
        <hr />
    </div>
    ...
    <div class="divTable2">
        <table cellpadding="0" cellspacing="0">
            <tr>
                ...
                <td>
                    <%if (Model.chc_status == 0)
                      { %>
                    <%=Html.DropDownList("chc_due_id", ViewData["userList"] as SelectList, "--请选择--", new {disabled="disabled" })%>
                    <%}
                      else
                      { %>
                    <%=Model.chc_due_id%>
