...
<body>
    <!--标题部分-->
    <div id="divTitle">
        <span class="title"><a href='<%=Url.Action("Index") %>'>客户信息管理</a>&nbsp;>>&nbsp;交往记录</span><br />
        <hr />
    </div>
    <div id="divTitleH">
    </div>
    ...
    <!--表头操作部分-->
    <div id="divListTop">
        <input type="button" onclick=javascript:window.location.href='<%=Url.Action("AddContactRecord",new{id=Model.cust_Id}) %>' class="transparent button" value="添加" />
        <input type="button" onclick=javascript:window.location.href='<%=Url.Action("Index") %>'; value="返回" class="transparent button" />
    </div>

    <!--列表部分-->
    <div id="divList">
        <% PageHelper<CRM.Model.cst_activity> pagerHelper = ViewData["pagerHelper"] as PageHelper<CRM.Model.cst_activity>;
        ...
        <table cellpadding="0" cellspacing="0">
            ...
            <tr class="tblItem">
                ...
                <td>
                    <%=Html.Encode(item.atv_date) %>
                </td>
                <td>
                    <%=Html.Encode(item.atv_place) %>
                </td>
                <td>
                    <%=Html.Encode(item.atv_title) %>
                </td>
                <td>
                    <%=Html.Encode(item.atv_desc) %>
                </td>
                <td>
                    <%=Html.Encode(item.aty_remark) %>
                </td>
                <td>
                    <a href='<%=Url.Action("UpdateContactRecord") %>/<%=Html.Encode(item.atv_id) %>'>
                        <img src="../../Images/ico/bt_edit.gif" title="编辑" class="imgLink" />
                    </a>
                    <a href='<%=Url.Action("DeleteContactRecord") %>/<%=Html.Encode(item.atv_id) %>'>
